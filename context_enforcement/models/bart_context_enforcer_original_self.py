...
class BartContextEnforcerLayer(nn.Module):
    def __init__(self,
        ...
        print('Adding cross context enforcement Bart Layer')
    ...
    def _compute_context(self, hidden_states: torch.FloatTensor,
        ...
        if attention_mask is not None:
            ...
            zc= torch.zeros(size=(left_context.shape[0], 1, fc_seq_len, 1),device=hidden_states.device)
...
class BartEncoderWithEnforcer(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        for idx in range(config.encoder_layers):
            if (idx+1)%2==0 and idx>1:
                self.layers.append(BartContextEnforcerLayer(config,
                                          is_normal_layer = idx<2))
            else:
                self.layers.append(BartEncoderLayer(config=config))
    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            is_normal_layer = not ((idx+1)%2==0 and idx>1)
            ...
            if self.training and (
                ...
                if self.gradient_checkpointing and self.training:
                    ...
                    ) if not is_normal_layer else torch.utils.checkpoint.checkpoint(
                        create_custom_forward(encoder_layer),
                        hidden_states,
                        attention_mask_,
                        (head_mask[idx] if head_mask is not None else None),
