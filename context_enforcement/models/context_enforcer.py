...
def compute_context_boundary(
    seq_max_len, context_sampling_bounds=(0.1, 0.45), context_max_len=100
):
    """
    ...
    :return:
    """
    boundary = np.random.uniform(context_sampling_bounds[0], context_sampling_bounds[1])
...
def split_contexts_with_boundary(
    hidden_rep,
    context_boundary,
):
...
def split_contexts_with_attention(
    attention_mask,
    context_boundary,
):
    boundary_start, boundary_end = context_boundary
...
class BartContextEnforcement(Module):
    def __init__(self, embed_dim, dropout=0.1, share_mha=False, num_heads=1):
        super().__init__()
    ...
    def forward(
        ...
        **kwargs
        ...
        left_focus, left_focus_attention, _ = self._left_mha(
            focus_context,
            left_context,
            layer_head_mask=layer_head_mask,
            output_attentions=output_attentions,
        )
        right_focus, right_focus_attention, _ = self._right_mha(
            focus_context,
            right_context,
            layer_head_mask=layer_head_mask,
            output_attentions=output_attentions,
        ...
        full_rep = torch.concat(
            [left_context, focus_combined_rep, right_context], dim=1
        )
        if not output_attentions:
            return [focus_combined_rep, full_rep], None
        else:
            return [focus_combined_rep, full_rep], [
                left_focus_attention,
                right_focus_attention,
            ]
...
class ContextEnforcementCrossed(Module):
    def __init__(
        ...
        if self._share_mha:
            mha = MultiheadAttention(dim, num_heads, dropout=dropout, batch_first=True)
            self._left_mha = mha
            self._right_mha = mha
        else:
            self._left_mha = MultiheadAttention(
                dim, num_heads, dropout=dropout, batch_first=True
            )
            self._right_mha = MultiheadAttention(
                dim, num_heads, dropout=dropout, batch_first=True
            )

        self._context_mha = MultiheadAttention(
            dim, num_heads, dropout=dropout, batch_first=True
        )
    ...
    def forward(self, hidden_rep, context_boundary: tuple, return_attentions=False):
        """
        Select the context focus and perform the context enforcement
        ...
        left_focus, left_focus_attention = self._left_mha(
            left_context, focus_context, focus_context
        )
        ...
        focus_rep, focus_attention = self._context_mha(focus_context, context, context)
        ...
        if not return_attentions:
            return [focus_rep, full_rep], None
        else:
